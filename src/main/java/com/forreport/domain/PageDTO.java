...
public class PageDTO {
	...
	public PageDTO(SearchingVO searchingVO, int total) {
		...
		/*
		 * 파라미터로 받아온 값으로 설정
		 * - totla(전체 게시글 개수), searchingVO(현재 페이지에 대한 정보를 담은 변수)
		 * 
		 * 그 외 생성자에서 설정하는 변수
		 * - endPage
		 * 		- 현재 페이지에서 10(한 페이지에 보여줄 페이지 번호의 개수)으로 나눈 후 * 10
		 * 		- 예시: 현재 페이지 12인 경우 > 12/10=2(올림처리) -> 2*10=20 -> 마지막 페이지 번호: 20
		 * - startPage
		 * 		- 현재 페이지에서 가장 처음에 보여줄 페이지 번호
		 * 		- endPage - 9
		 * 
		 * 기타 사용하는 변수
		 * - realEnd
		 * 		- 혹시 전체 페이지 개수가 35개여야 하는데 현재 페이지가 32인 경우 endPage가 40으로 되는데,
		 * 			이 경우를 35로 바꿔주는 변수
		 * 		- 전체 게시글이 343개이고, amount가 10인 경우 --> realEnd 34P를 endPage로 지정
		 * 
		 */
